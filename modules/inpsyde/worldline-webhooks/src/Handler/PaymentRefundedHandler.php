...
class PaymentRefundedHandler implements WebhookHandlerInterface
{
    ...
    public function handle(WebhooksEvent $webhook, WlopWcOrder $wlopWcOrder) : void
    {
        ...
        try {
            ...
            /* translators: 1 CAWL transaction ID */
            $wlopWcOrder->addWorldlineOrderNote(\sprintf(\__('Failed to issue a refund in the WooCommerce. Transaction ID: %s', 'cawl-for-woocommerce'), WebhookHelper::transactionIdForUi($webhook)));
    ...
    protected function makeReasonText(WebhooksEvent $webhook) : string
    {
        ...
        if ($webhook->type === 'payment.refunded') {
            $reason = \__('Refund processed.', 'cawl-for-woocommerce');
        } elseif ($webhook->type === 'payment.cancelled') {
            $reason = \__('Authorization cancelled.', 'cawl-for-woocommerce');
        }
        return (string) \sprintf(
            /* translators: %s transaction ID */
            \__('%1$s CAWL transaction ID: %2$s', 'cawl-for-woocommerce'),
            $reason,
    ...
    protected function issueRefund(WebhooksEvent $webhook, WlopWcOrder $wlopWcOrder) : void
    {
        ...
        if ($webhook->type === 'payment.refunded') {
            ...
            $note = \__('%s was refunded.', 'cawl-for-woocommerce');
        } elseif ($webhook->type === 'payment.cancelled') {
            /* translators: 1 Amount of money */
            $note = \__('%s was cancelled.', 'cawl-for-woocommerce');
        }
