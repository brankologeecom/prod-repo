...
document.addEventListener( 'DOMContentLoaded', () => {
	const chkLiveMode = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_live_mode'
	) as HTMLInputElement | null;

	const lstAuthorizationMode = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_authorization_mode'
	) as HTMLInputElement | null;

	const chkIs3dsAuthentication = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_enable_3ds'
	) as HTMLInputElement | null;

	const chkIs3dsExemption = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_request_3ds_exemption'
	) as HTMLInputElement | null;

	const lst3dsExemptionType = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_3ds_exemption_type'
	) as HTMLInputElement | null;

	const num3dsExemptionLimit = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_3ds_exemption_limit'
	) as HTMLInputElement | null;
	...
	function updateLiveTestFields() {
		...
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_live_api_key'
			),
			isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_live_api_secret'
			),
			isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_live_api_endpoint'
			),
			isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_live_webhook_id'
			),
			isLive,
			'wlop-hidden'
		);
		...
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_test_api_key'
			),
			! isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_test_api_secret'
			),
			! isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_test_api_endpoint'
			),
			! isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_test_webhook_id'
			),
			! isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_test_webhook_secret_key'
			),
			! isLive,
			'wlop-hidden'
		);
	...
	function updateAuthorizationFields() {
		...
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_credit_card_authorization_mode'
			),
		...
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_capture_mode'
			),
			lstAuthorizationMode.value === 'authorization',
			'wlop-hidden'
		);
	...
	function update3dsFields() {
		...
		setVisibleByClass(
			getFieldRow(
				'#woocommerce_cawl-for-woocommerce_enforce_3dsv2'
			),
		...
		setVisibleByClass(
			'#woocommerce_cawl-for-woocommerce_3ds_exemption_warning',
			chkIs3dsAuthentication.checked && chkIs3dsExemption.checked,
			'wlop-hidden'
		);
	}
