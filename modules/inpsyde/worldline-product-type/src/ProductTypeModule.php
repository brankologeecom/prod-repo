...
namespace Syde\Vendor\Cawl\Inpsyde\WorldlineForWoocommerce\ProductType;
...
class ProductTypeModule implements ExecutableModule, ServiceModule
{
    ...
    {
        \add_meta_box('worldline-product-type', \__('CAWL Direct', 'cawl-for-woocommerce'), [$this, 'renderProductMetaBox'], 'product', 'side', 'core');
    }
    /**
    ...
    public function renderProductMetaBox($post) : void
    {
        ...
        <p><label for="worldline_mealvoucher_type">
                <?php 
        \_e('CAWL Mealvouchers Product Type:', 'cawl-for-woocommerce');
        ?>
            </label></p>
        ...
        ?>><?php 
        \_e('None', 'cawl-for-woocommerce');
        ?></option>
            <option value="food" <?php 
        \selected($current_type, 'food');
        ?>><?php 
        \_e('Food and drink', 'cawl-for-woocommerce');
        ?></option>
            <option value="home" <?php 
        \selected($current_type, 'home');
        ?>><?php 
        \_e('Home and garden', 'cawl-for-woocommerce');
        ?></option>
            <option value="gift" <?php 
        \selected($current_type, 'gift');
        ?>><?php 
        \_e('Gift and flowers', 'cawl-for-woocommerce');
        ?></option>
        </select>
    ...
    public function ajaxSaveProductType() : void
    {
        ...
        if ($type === 'none') {
            ...
            \wp_send_json_success(['message' => \__('Product type removed.', 'cawl-for-woocommerce')]);
        }
        ...
        \wp_send_json_success(['message' => \__('Product type saved successfully.', 'cawl-for-woocommerce')]);
    }
}
